<?php   header('Content-Type: text/html; charset=utf-8');
        ...
        $utm_source = isset($_SESSION['utmSource']) ?  $_SESSION['utmSource'] : "";
		...
		$pagina = isset($_POST['pagina']) ? $_POST['pagina'] : ''; include "conexao.php";
		...
		$sql = mysqli_query($conecta, "SELECT conteudo.id as id_conteudo, conteudo.*, tconteudo.tipo from TB_conteudo as conteudo 
										...
										WHERE (conteudo.situacao = 1 or conteudo.situacao = 3) && pagina.link = '".$_POST['pagina']."' ".$pesquisa_filtro."  GROUP BY TB_append_conteudo.vinculo_conteudo  order by conteudo.data_cadastro DESC ".$limit);
...
<?php 		}else{ ?>
		<p style="letter-spacing: 1px !important; font-size: 12pt !important; font-family: sans-serif !important; line-height: 1.9 !important; font-weight: 400 !important; text-align: center !important;"><?php if(strlen(utf8_encode($array_append['descricao']) )> 120) {echo substr(utf8_encode($array_append['descricao']), 0, 180)."...";} else{echo utf8_encode($array_append['descricao']);} ?></p>			
<?php		}
...
<?php				}
		...
		$sql = mysqli_query($conecta, "SELECT conteudo.id as id_conteudo, conteudo.*, tconteudo.tipo from TB_conteudo as conteudo 
										...
										INNER JOIN TB_pagina as pagina on pagina.id = vinculo.id_pagina
										INNER JOIN TB_append_conteudo ON TB_append_conteudo.vinculo_conteudo = conteudo.id
										WHERE (conteudo.situacao = 1 or conteudo.situacao = 3) && pagina.link = 'blog-justa.html' ".$pesquisa_filtro." GROUP BY TB_append_conteudo.vinculo_conteudo  order by conteudo.data_cadastro DESC");
...
<?php				}
		...
		$sql = mysqli_query($conecta, "SELECT conteudo.id as id_conteudo, conteudo.*, tconteudo.tipo from TB_conteudo as conteudo 
										...
										INNER JOIN TB_pagina as pagina on pagina.id = vinculo.id_pagina
										INNER JOIN TB_append_conteudo ON TB_append_conteudo.vinculo_conteudo = conteudo.id
										WHERE (conteudo.situacao = 1 or conteudo.situacao = 3) && pagina.link = 'blog-justa.html' ".$pesquisa_filtro." ");
...
<?php				}
		...
		if($pagina=="cadastro-email"){
			...
			$sql = mysqli_query($conecta, "SELECT id from TB_cadastros_adquiridos WHERE nome = '".$nome."' and email = '".$email."'");
			if(mysqli_num_rows($sql)>0){echo trim("Esse e-mail já existe em nossa base de dados. Obrigado");} 
			else{
				$sql = mysqli_query($conecta, "INSERT into TB_cadastros_adquiridos (nome, email, celular, data_cadastro, vinculo_site) VALUES ('".$nome."', '".$email."', '".$celular."', '".date('Y-m-d')."', 0)");
				if($sql){
...
<?php		} 
	...
	$origin = $utm_source; // isset($_POST['utmSource']) ? $_POST['utmSource'] : '';
	$pagina_origem = utf8_decode($_POST['pagina_origem']);
	//$emailenviar = "brugazzo@hotmail.com";

	$sql = mysqli_query($conecta, "SELECT id from TB_cadastros_interesse WHERE nome = '$nome_contato' and email = '$email_contato'");
    
    if(mysqli_num_rows($sql)<=0){ 
            $sql = mysqli_query($conecta, "INSERT into TB_cadastros_interesse (nome, email, telefone, cidade, estado, obs, data_cadastro, teste, link_origem, pagina_origem) VALUES".
        " ('$nome_contato', '$email_contato', '$fone_contato', '$cidade_contato', '$estado_contato', '', '".date('Y-m-d')."', 'S', '$origin', '$pagina_origem')");
            ...
            if($sql)
            {
				...
				echo 'Você já possui cadastro como interessado.';
			}
	...
	if($pagina=="cadastro-interesse"){
            //isset($_POST['utmSource']) ? $_POST['utmSource'] : ''
			$origin = strlen($utm_source) > 0 ? $utm_source : '';
			$pagina_origem = isset($_POST['pagina_origem']) ? utf8_decode($_POST['pagina_origem']) : '';		
			$canal = isset($_POST['canal']) ? utf8_decode($_POST['canal']) : '';	
            $referral = isset($_POST['referral']) ? utf8_decode($_POST['referral']) : '';	
            

			$sql = mysqli_query($conecta, "SELECT id from TB_cadastros_interesse WHERE nome = '".$_POST['nome']."' and email = '".$_POST['email']."'");
			if(mysqli_num_rows($sql)>0){
                //echo trim("Esse e-mail já existe em nossa base de dados. Obrigado");
                print(json_encode(array('error' => true, 'message' => 'Esse e-mail já existe em nossa base de dados. Obrigado')));
            } 
			else{
				$sql_command = "INSERT into TB_cadastros_interesse (nome, email, telefone, cidade, estado, obs, data_cadastro, teste, pagina_origem, link_origem, canal) VALUES ('".$_POST['nome']."', '".$_POST['email']."', '".$_POST['telefone']."', '".$_POST['cidade']."', '".$_POST['estado']."', '".$_POST['obs']."', '".date('Y-m-d')."', 'S', '$pagina_origem', '$origin', '$canal')";
				$sql = mysqli_query($conecta, $sql_command );
				...
				if($sql){
					...
					<br><br>Nome: ".$nome_contato.";<br>
							...
							Telefone: ".$fone_contato.";<br>
							Estado: ".$estado_contato.";<br>
							Cidade:  ".$cidade_contato.";<br><br>
							Obs: Teste Homolog. (Desconsiderar)";
					$headers = 'MIME-Version: 1.0' . "\r\n";
					$headers .= 'Content-type: text/html; charset=iso-Utf-8' . "\r\n";
					$headers .= 'From: '.$nome_contato.' <'.$email_contato.'>';
					$enviaremail = mail( $destino, $assunto, $mensagem, $headers );
					if ( $enviaremail ) {
                        //echo "E-mail enviado com sucesso!. Aguarde nosso contato.";
                        print(json_encode(array('error' => false, 'message' => 'E-mail enviado com sucesso!. Aguarde nosso contato.')));
					} else {
                        //echo "Falha ao enviar o e-mail, tente novamente";
                        print(json_encode(array('error' => false, 'message' => 'Falha ao enviar o e-mail, tente novamente')));
					}					
				}
			}
}//FECHA INTERESSE

if($pagina == "cadastro-cartao"){
		
	$nome_contato = utf8_decode($_POST['nome_contato']);
	$email_contato = strtolower(utf8_decode($_POST['email_contato']));	
	$fone_contato = $_POST['fone_contato'];	
	$estado_contato = utf8_decode($_POST['estado_contato']);
	$cidade_contato = utf8_decode($_POST['cidade_contato']);
	$obs_contato = utf8_decode($_POST['obs_contato']);	
	$origin = $utm_source; // isset($_POST['utmSource']) ? $_POST['utmSource'] : '';
	$pagina_origem = utf8_decode($_POST['pagina_origem']);
	//$emailenviar = "brugazzo@hotmail.com";
    ...
    if(mysqli_num_rows($sql)<=0){ 
            $sql = mysqli_query($conecta, "INSERT into TB_cadastros_interesse (nome, email, telefone, cidade, estado, obs, data_cadastro, teste, link_origem, pagina_origem) VALUES".
        " ('$nome_contato', '$email_contato', '$fone_contato', '$cidade_contato', '$estado_contato', '$obs_contato', '".date('Y-m-d')."', 'S', '$origin', '$pagina_origem')");
            ...
            if($sql)
            {
				...
				if ( $enviaremail ) {
					echo "Cadastrado com sucesso. Logo entraremos em contato com você!";
				} else {
					echo "Falha ao enviar cadastrar";
				}
			}

			}else{
				echo 'Você já possui cadastro na Justa.';
			}
	
			
		}

if($pagina == "cadastro-linkou"){
	...
	$estado_contato = utf8_decode($_POST['estado_contato']);
	$cidade_contato = utf8_decode($_POST['cidade_contato']);
	$origem_contato = utf8_decode($_POST['origem_contato']);
	$obs_contato = utf8_decode($_POST['obs_contato']);	
	$origin = $utm_source; // isset($_POST['utmSource']) ? $_POST['utmSource'] : '';
	$pagina_origem = utf8_decode($_POST['pagina_origem']);
	//$emailenviar = "brugazzo@hotmail.com";
    ...
    if(mysqli_num_rows($sql)<=0){ 
            $sql = mysqli_query($conecta, "INSERT into TB_cadastros_interesse (nome, email, telefone, cidade, estado, obs, data_cadastro, teste, link_origem, canal, pagina_origem) VALUES".
        " ('$nome_contato', '$email_contato', '$fone_contato', '$cidade_contato', '$estado_contato', '$obs_contato', '".date('Y-m-d')."', 'S', '$origin', '$origem_contato', '$pagina_origem')");
