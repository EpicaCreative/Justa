<?php include "cabecalho.php"; $pagina_atual = $_GET['pagina']; if($pagina_atual=="") $pagina_atual = 1;
	if($_GET['acao']=="excluir"){
		...
		if($Delete){
			echo "<script>alert('Deletado com sucesso'); window.location='listagem_downloads.php';</script>";
			}
	}
?>
                <div class="span9"      >
            ...
            <div class="row-fluid pull">
                    <h2 class="page-header">Itens Cadastrados&nbsp;<a href="links_download.php" class="btn btn-success">Cadastrar</a></h2>
                    ...
                    <div class="row-fluid pull">
                    	...
                    	<div class="span3"> 
                    		<select name="classe" onChange="this.form.submit();">
                     			<option <?php if($_GET['classe']==""){echo "selected"; } ?> value="">Todos os tipos</option> 
                     			<?php $select_col2 = mysqli_query(conexao(),"Select classe from menus_download where classe <> 'Informativos' GROUP by menus_download.classe Desc");
								while($array_col2 = mysqli_fetch_array($select_col2)){ ?>
                    			                  	<option <?php if($_GET['classe']==utf8_encode($array_col2['classe'])){echo "selected"; } ?> value="<?php echo utf8_encode($array_col2['classe']); ?>"><?php echo utf8_encode($array_col2['classe']); ?></option>
							<?php } ?>   
                    		</select>
                    	</div>
                    	<div class="span3"> 
...
<?php 			$registros = 0;
				...
				?>
						                <tr>
						                  ...
						                  <td><?php echo utf8_encode($array['classe']); ?></td>
						                  <td><?php echo utf8_encode($array['descricao']); ?></td>
						                  <td><a href="dados_download.php?id=<?php echo $array['id'] ?>" class="btn btn-success">Editar</a></td>
						                  <td><a href="listagem_downloads.php?id=<?php echo $array['id'] ?>&acao=excluir" class="btn btn-danger">Deletar</a></td>
						                </tr>
										...
										<div class="col-md-6">
											<nav style="float: right">
												<ul class="pagination pagination-lg">
													<li>
														<a href="?data_cadastro=<?php echo $_GET['data_cadastro'] ?>&classe=<?php echo $_GET['classe'] ?>&descricao=<?php echo $_GET['descricao'] ?>&pagina=1" class="paginacao-submit">«</a>
													</li>
													<?php 
										 for($anterior = $pagina_atual - 3; $anterior < $pagina_atual; $anterior++){
										 if($anterior > 0 && $anterior != $pagina_atual){ ?>
													<li>
														<a href="?data_cadastro=<?php echo $_GET['data_cadastro'] ?>&classe=<?php echo $_GET['classe'] ?>&descricao=<?php echo $_GET['descricao'] ?>&pagina=<?php echo $anterior ?>" class="paginacao-submit"><?php echo $anterior ?></a>
													</li>
													<?php } } ?>
													<li>
													...
													for ( $proxima = $pagina_atual + 1; $proxima < $pagina_atual + 4; $proxima++ ) {
														if ( $proxima <= $total_paginas ) {
															?>
													<li>
														<a href="?data_cadastro=<?php echo $_GET['data_cadastro'] ?>&classe=<?php echo $_GET['classe'] ?>&descricao=<?php echo $_GET['descricao'] ?>&pagina=<?php echo $proxima ?>" class="paginacao-submit"><?php echo $proxima ?></a>
													</li>
													<?php }} ?>
													<li>
														<a href="?data_cadastro=<?php echo $_GET['data_cadastro'] ?>&classe=<?php echo $_GET['classe'] ?>&descricao=<?php echo $_GET['descricao'] ?>&pagina=<?php echo $total_paginas ?>" class="paginacao-submit">»</a>
													</li>
												</ul>
											</nav>
										</div>
