<?php 
    ...
    $append = " WHERE pagina_origem='seja-um-justo.html' AND (month(data_cadastro) = $month OR $month=0) AND (YEAR(data_cadastro) = $year OR $year = 0) ";
    if(strlen($query) > 0)
    {
    ...
    $sql_search_justos = "SELECT * FROM TB_cadastros_interesse $append ORDER BY id DESC LIMIT 25 OFFSET $offset";

    //print($sql_search_justos);

    $statement = mysqli_query(conexao(), $sql_search_justos);
    
    $sql_search_pages_justos = "SELECT * FROM TB_cadastros_interesse WHERE pagina_origem='seja-um-justo.html' ";
    
    $sql_total_rows = mysqli_query(conexao(), $sql_search_pages_justos);
    $registros = 0;
    $total_registros = $sql_total_rows->num_rows;//mysqli_num_rows($sql_total_rows);
    
    
    $total_paginas = ceil($total_registros / 25) <= 0 ? 1 : ceil($total_registros / 25);
    
    $current_year = date("Y");
    
    // print_r($sql_total_rows);
    // print("total páginas $total_paginas");
?>
<div class="span9" >     
                
                <div class="row-fluid pull">
                <h2 class="page-header" style="text-align: left;">Justos/Interessados Cadastrados&nbsp;
                ...
                <form method="GET">
                    <div class="row-fluid pull">
                        ...
                        <div class="span4">                                                      
                            <a href="export/listagem_justos.php?month=<?php echo $month; ?>&year=<?php echo $year; ?>&query=<?php echo $query; ?>" class="btn btn-success">Exportar (Excel)</a>
                                          ...
                                          <tbody>
                                                ...
                                                while($array = mysqli_fetch_array($statement)){ 
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><?php echo utf8_encode($array['link_origem']); ?></td>
                                                    </tr>
                                                <?php
                                                    $registros++;
                                                } ?>
